...
```bash
python main.py --input src/main.cpp --output translated/ --mode flexible --verbose
```

Available flags:
- `--input/-i`: Input C++ file(s) or directory(ies) to convert (required)
- `--output/-o`: Output directory for generated Java files (required)
- `--mode`: Conversion mode: strict or flexible (default: strict)
- `--report`: Generate translation report to specified file (JSON or TXT)
...
```bash
# Convert single file
python main.py --input myfile.cpp --output output/

# Convert directory with flexible mode
python main.py --input src/ --output translated/ --mode flexible

# Generate report
python main.py --input src/ --output out/ --report report.json --verbose
```
